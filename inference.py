...
def download_file_from_s3(url, local_path):
    ...
    if response.status_code == 200:
        ...
        raise print("Failed to download")
...
def main(image_url, audio_url):
    ...
    if not wav_local_path.endswith('.wav'):
        command = 'ffmpeg -loglevel error -y -i {} -strict -2 {}'.format(wav_local_path,
                                                                         'temp/{}/temp.wav'.format(args.tmp_dir))
        subprocess.call(command, shell=True)
        wav_local_path = 'temp/{}/temp.wav'.format(args.tmp_dir)
    wav = audio.load_wav(wav_local_path, 16000)
    mel = audio.melspectrogram(wav)
    if np.isnan(mel.reshape(-1)).sum() > 0:
        raise ValueError('Mel contains nan! Using a TTS voice? Add a small epsilon noise to the wav file and try again')
